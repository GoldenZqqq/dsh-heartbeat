	...
	factory: (require) => {
		...
		*/
		...
		var HeartbeatEngine = class {
			...
			/** Push one decoded session event into the engine. */
		...
		*/
		...
		function HeartbeatWidget({ engine, wide = true, floating = false, useSessions }) {
			...
			const [floatingPos, setFloatingPos] = (0, react.useState)(() => {
				try {
					const raw = localStorage.getItem("dsh-heartbeat:pos");
					if (raw) {
			...
			const onFloatPointerDown = (0, react.useCallback)((e) => {
			...
			const onFloatPointerMove = (0, react.useCallback)((e) => {
				...
				const x = Math.min(Math.max(0, e.clientX - d.dx), vw - rect.width);
				const y = Math.min(Math.max(0, e.clientY - d.dy), vh - rect.height);
				setFloatingPos({ left: x, top: y });
			}, []);
			const onFloatPointerUp = (0, react.useCallback)((e) => {
				const d = dragRef.current;
				if (!d || e.pointerId !== d.pointerId) return;
				...
				try {
					...
					const rect = el.getBoundingClientRect();
					localStorage.setItem("dsh-heartbeat:pos", JSON.stringify({ left: rect.left, top: rect.top }));
				} catch {}
			}, []);
			(0, react.useEffect)(() => {
			...
			if (floating) return (0, react_jsx_runtime.jsx)("div", {
				...
				style: floatingPos ? { left: `${floatingPos.left}px`, top: `${floatingPos.top}px`, right: "auto", bottom: "auto" } : void 0,
		...
		/** Required services:the stream connection, session list, remote session follow,andthe layout slot system. */
		const inject = ["connection", "slots", "sessions", "remote", "remote.session"];
		/**
		* Client plugin body: track running sessions through the session controller,
		* open follow streams for their token events, start the engine, register the
		* heartbeat widget into the shell.overlay floating layer. Drag position persists
		* in localStorage.
		...
		*/
		function apply(ctx) {
			...
			let disposed = false;
			const streamAborts = /* @__PURE__ */ new Map();
			...
			const syncConnected = () => {
				...
				engine.connected = state === "connected";
				if (state !== "connected") {
					for (const ac of streamAborts.values()) ac.abort();
					streamAborts.clear();
					startedIds.clear();
				} else {
					syncSessionStreams();
			...
			syncConnected();
			const offState = connection.state.subscribe(syncConnected);

			function runSessionStream(address, signal) {
				(async () => {
					try {
						for await (const frame of remote.session.follow({ address }, signal)) {
							if (disposed || signal.aborted) break;
							if (frame.type === "event") engine.handleSessionEvent(frame.event);
					...
					const key = address.kind === "session" ? address.sessionId : address.childSessionId;
					if (!disposed && !signal.aborted) {
						startedIds.delete(key);
						streamAborts.delete(key);
						setTimeout(syncSessionStreams, 1500);
					} else {
						startedIds.delete(key);
						streamAborts.delete(key);
					}
			...
			function syncSessionStreams() {
				if (disposed) return;
				const snap = sessions.list.getSnapshot();
				for (const id of snap.ids) {
					const row = snap.byId[id];
					if (row?.running === true && !startedIds.has(id)) {
						let address = { kind: "session", sessionId: id };
						if (row.origin === "subagent") {
							const addr = sessions.subagentAddress(id);
							if (!addr) continue;
							address = addr;
						}
						startedIds.add(id);
						const ac = new AbortController();
						streamAborts.set(id, ac);
						runSessionStream(address, ac.signal);
					}
				}
				for (const [id, ac] of [...streamAborts]) {
					if (!snap.byId[id] || !snap.byId[id].running) {
						ac.abort();
			...
			syncSessionStreams();
			const offSessions = sessions.list.subscribe(syncSessionStreams);
